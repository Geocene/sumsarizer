...
read.ibutton <- function(input_file, timezone = "UTC", lablr_output = T) {
	...
	time_interval <- unique(diff(as.numeric(parse_date_time(paste(Sys.Date(),(sapply(strsplit(time_test$timestamp, " "), '[[', 2))), orders=time_format))))/60
